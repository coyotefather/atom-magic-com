...
interface SharedCharacterPageContentProps {
...
export default function SharedCharacterPageContent({
	cultures,
	paths,
	patronages,
	disciplines,
}: SharedCharacterPageContentProps) {
	...
	const cultureName = character?.culture
		? cultures.find(c => c._id === character.culture)?.title || character.culture
		: null;
	const pathName = character?.path
		? paths.find(p => p._id === character.path)?.title || character.path
		: null;
	const patronageData = character?.patronage
		? patronages.find(p => p._id === character.patronage)
		: null;
	const patronageName = patronageData
		? `${patronageData.title}${patronageData.epithet ? `, ${patronageData.epithet}` : ''}`
		: character?.patronage || null;

	// Get discipline and technique names
	const getDisciplineName = (id: string): string => {
		const discipline = disciplines.find(d => d._id === id);
		return discipline?.title || id;
	};
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
					...
					<div className="bg-white dark:bg-black border-2 border-stone p-6">
						...
						<div className="space-y-3">
							...
							{!cultureName && !pathName && !patronageName && (

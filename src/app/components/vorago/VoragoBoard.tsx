 ...
 * VoragoBoard.tsx
 *
 * The main SVG board renderer for the Vorago game. Draws the five concentric
 * rings of the board as interactive pie-slice cells, overlaid with stones,
 * walls, bridges, lock icons, and the central goal.
 *
 * --- Ring / Cell Structure ---
 * Five rings are defined in RING_CONFIG (outermost = index 0, innermost = 4):
 *   Ring 0: 32 cells, radius 250px   (outermost)
 *   Ring 1: 16 cells, radius 212.5px
 *   Ring 2: 16 cells, radius 175px
 *   Ring 3:  8 cells, radius 136.5px
 *   Ring 4:  4 cells, radius 100px   (innermost, touching the goal)
 *
 * Each cell is a donut-slice SVG path generated by `generatePieSlicePath`,
 * which computes arc coordinates from the ring's cell count, radius, and the
 * cell index. The innermost ring uses a solid pie-slice to the centre.
 *
 * All 76 pie-slice paths and cell-centre coordinates are pre-computed at
 * module load time (PIE_SLICE_PATHS, CELL_CENTERS) so they are not
 * recalculated on every render.
 *
 * --- Rotation ---
 * Each ring is wrapped in a Framer Motion `<motion.g>` element that animates
 * its CSS `rotate` property to the current `degrees[ringIndex]` value from
 * Redux state. Adjacency between cells in different rings is calculated
 * angularly using `areCellsAdjacent`, which accounts for the current rotation
 * of both rings so legal move validation stays correct after a ring spins.
 *
 * --- Click Handling ---
 * A single `handleCellClick(ring, visualCell)` callback handles all clicks on
 * the board. Its behaviour depends on the current Redux selection state:
 *   - If a stone is selected  → attempt to move it to the clicked cell, running
 *     wall/adjacency/occupancy checks and special bridge jump logic.
 *   - If a coin is active     → apply the coin's board effect (place/remove wall
 *     or bridge, transform, move with Spectrum's two-step pick).
 *   - Otherwise               → select the stone on the clicked cell (if it
 *     belongs to the active player and the player hasn't moved yet).
 * Clicking the central goal circle is treated as ring 5, cell 0.
 *
 * --- BoardCell sub-component ---
 * Each cell is rendered by a memoised `BoardCell` to prevent unnecessary
 * re-renders when only one cell's state changes. It renders: the pie-slice
 * background, an optional wall (dark square) or bridge (green lines), an
 * animated stone circle, a pulsing gold ring when a stone is selected, and —
 * in debug mode — the cell index label.
 *
 * --- DEBUG_MODE ---
 * Controlled by the NEXT_PUBLIC_VORAGO_DEBUG environment variable (set to
 * "true" to enable). When active, a floating overlay shows the rotation
 * degree of each ring in real time, and each cell displays its logical cell
 * index as a text label. Useful for verifying adjacency, rotation maths, and
 * AI move targeting.
 *
 * Used by:
